...
const QuizNewBlockSidebar = ({
	...
	addInput: (
		input:
			...
			| (typeof QuizPrebuiltBlocks)[number]["inputs"]
	) => void;
}) => {
	...
	const FilteredPrebuiltBlocks = useMemo(() => {
		const items = QuizPrebuiltBlocks;
		if (search?.length > 1) {
			return items.filter(
				(item) =>
					item.title
	...
	return (
		<div className="">
			...
			{QuestionBlocks?.length > 0 && (
				<>
					<p className="text-sm text-gray-500 my-3">Questions</p>
					<div className="grid grid-cols-2 gap-2">
						{QuestionBlocks.map((input, index) => (
							<InputBlock
...
export default QuizNewBlockSidebar;
