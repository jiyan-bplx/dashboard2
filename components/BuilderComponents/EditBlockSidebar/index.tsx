...
import YoutubeEmbedOptions from "./YoutubeEmbedOptions";
import GoogleAnalyticsOptions from "./GoogleAnalyticsOptions";
import { CheckIcon } from "@heroicons/react/20/solid";
import { Key } from "react";

const EditBlockSidebar = ({ isAvailable }: { isAvailable?: boolean }) => {
			...
			<div className="px-8 h-full min-h-full">
							...
							(!["checkbox", "recaptcha", "switch"].includes(
								...
								<div className="mt-3 mb-3">
									...
									<p className="text-xs text-gray-400 mt-2">
										Prevent submission if this field is
										empty
									</p>
								</div>
							) : (
				...
				{selectedInput.component === "input" &&
					selectedInput.type === "picture_choice" && (
						<PictureChoiceInputOptions activeTab={activeTab} />
					)}

				{selectedInput.component === "input" &&
					selectedInput.type === "tel" && (
						<PhoneInputOptions activeTab={activeTab} />
					)}

				{selectedInput.type === "file" && (
					<FileUploadOptions activeTab={activeTab} />
				)}

				{selectedInput.component === "special_input" &&
					selectedInput.type === "cal" && (
						<CalOptions activeTab={activeTab} />
					)}

				{selectedInput.component === "special_input" &&
					selectedInput.type === "calendly" && (
						<CalendlyOptions activeTab={activeTab} />
					)}

				{/* {selectedInput.component === "special_input" &&
					selectedInput.type === "razorpay_payment_button" && (
						<RazorpayPaymentButtonOptions activeTab={activeTab} />
					)} */}

				{selectedInput.component === "special_input" &&
					selectedInput.type === "google_analytics" && (
						<GoogleAnalyticsOptions activeTab={activeTab} />
					)}

				{selectedInput.component === "layout" && (
					<>
						{selectedInput.type === "page_title" && (
							<PageTitleOptions activeTab={activeTab} />
						)}
						{selectedInput.type === "gmaps" && (
							<GoogleMapsEmbedOptions activeTab={activeTab} />
						)}
						{selectedInput.type === "youtube" && (
							<YoutubeEmbedOptions activeTab={activeTab} />
						)}
						{selectedInput.type === "text" && (
							<div className="mt-2">
								<label
				...
				{activeTab === "design" && (
					<>
						...
						>
							<Select
								readOnly={selectedInput.display === false}
								placeholder={
									selectedInput.component === "layout"
										? "Block width"
										: "Input width"
								...
								onChange={(val) => {
									if (selectedInput.display === false) {
										return;
									}
									changeProperty({
										key: "width",
...
export default EditBlockSidebar;
